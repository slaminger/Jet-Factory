...
docker run --name jet --privileged --cap-add=ALL --device=/dev/fuse --security-opt apparmor:unconfined --rm -it -v "$basepath":/root/linux -v /var/run/docker.sock:/var/run/docker.sock alizkan/jet-factory:1.0.0
