...
Main() {
	...
	if [[ $(file -b --mime-type "${dl_dir}/${img}") == "application/octet-stream" ]]; then
		if [[ $(file -b "${dl_dir}/${img}" | sed -E 's/ID=0x8e//g') == 0 ]]; then
			offset=$(($(file -b "${dl_dir}/${img}" | sed -E 's/^.*(ID=0x8e.*startsector.*),.*$/\1/' | rev | cut -d" " -f1 | rev) * 512))
			vgchange -ay ${selection}
		elif [[ $(file -b "${dl_dir}/${img}" | sed -E 's/ID=0x83e//g') == 0 ]]; then
			offset=$(($(file -b "${dl_dir}/${img}" | sed -E 's/^.*(ID=0x83.*startsector.*),.*$/\1/' | rev | cut -d" " -f1 | rev) * 512))
		else
			echo "Can't recognize partition type exiting ..."
			exit 1
		fi
